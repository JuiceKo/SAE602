...
#Déploiement
docker network create --driver overlay reseau_sae

docker build -t api -f Dockerfile .
docker build -t mysql -f Dockerfile_mysql .

docker service create --name api_python --network reseau_sae api
docker service create --name database --network reseau_sae mysql
